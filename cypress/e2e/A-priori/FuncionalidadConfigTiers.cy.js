...
describe('Tester de funcionalidad tiers de config', () => {
    ...
    it('E0004 Modificando tier free', () => {
        ...
        cy.fixture('A-priori/tiers.json').then((tier) => {
            const randomIndex = Math.floor(Math.random() * tier.length);
            const tierSelect = tier[randomIndex];
            cy.get('input[id=":r16:"]').clear().type(tierSelect.FullName)
            cy.get('input[id=":r18:"]').clear().type(tierSelect.TierDescription)
